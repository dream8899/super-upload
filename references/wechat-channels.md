...
- **位置默认不显示地址**：上传页「短标题」后的「位置」字段保持为空，不显示任何地址；
  只有用户明确要求填写时才设置。
- **封面生成与网络环境相关（先检查再决策）**：上传后先检查封面是否正常生成、发表按钮是否
  可用；正常则直接发表。卡住时：若检测到 VFR（`r_frame_rate` ≠ `avg_frame_rate`）先重编码为
  恒定帧率 + faststart 再传（结合网络环境判断）；仍卡住再走「保存草稿 → 草稿箱 → 定时发表」
  兜底。不要无条件重编码。
- **视频标注选「含AI生成内容」（必检）**：本工作流产出的视频默认含 AI 生成内容，上传页
  「视频标注」默认选择「含AI生成内容」并回读确认；未确认通过不得保存草稿/发表。
- **不自动选择合集**：除非用户明确要求，上传时不选「添加到合集」。
- **标签不含比例描述**：去除「1:64 / 1比64」等比例词，避免错误规格标注。
- **定时只能设未来 10 天**：超过 10 天的排班必须按 10 天窗口分批执行。
- **排班默认保存草稿即完成**：排班计划执行以草稿箱出现对应标题为完成判据；定时发表需
  用户另行明确授权。发布前必须先完成视频与文案的逐条核对（可先生成带文件名的截图墙
  供人工审阅），未核验文案不得发表。
